...
async function fetchIndicatorsFor(sym:string, source:'binance'|'coingecko') {
  const t = (tokens as any).tokens.find((x:any)=>x.symbol?.toUpperCase()===sym);
  if (!t) return null;
...
export default function ChatBot(){
  ...
  const [messages,setMessages]=useState<Msg[]>([
    { role:'assistant', content:'👋 Salut ! Je suis MyAiTraderBot. Pose-moi une question. J’essaierai d’agréger les indicateurs automatiquement.' }
  ]);
  ...
  async function send(msg?:string){
    ...
    try{
      ...
      Object.values(existing).forEach((v:any)=>{
        if (v?.source===source && v?.indicators) ctxList.push({ symbol: (v.symbol||'').toUpperCase(), source, price: v.price, indicators: v.indicators });
      });
      ...
      setMessages(m=>[...m, {role:'assistant', content: '❌ ' + (e?.message || 'Assistant désactivé ou erreur API.')}]);
