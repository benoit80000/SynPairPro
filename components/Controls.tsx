...
type Props = { onChange?: (opts: { source: "binance" | "coingecko"; intervalMs: number }) => void };
export default function Controls({ onChange }: Props) {
  const [source, setSrc] = useState<"binance" | "coingecko">(getSource());
    ...
    <div className="mb-4 flex flex-wrap items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="flex items-center gap-2">
        ...
        <select className="input" value={source}
          onChange={(e) => { const v = (e.target.value as "binance" | "coingecko"); setSrc(v); setSource(v); onChange?.({ source: v, intervalMs }); }}>
